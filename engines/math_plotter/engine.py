"""Pure parabolic axis transformation engine.
...
Self-contained with no Streamlit and no other engines.
Plotting backends live in :mod:`engines.math_plotter.plots`
(data / Matplotlib PNG / Plotly HTML + local server).
"""
...
class Axis:
    """Matplotlib wrapper around the pure functions above.

    New code should prefer :mod:`engines.math_plotter.plots`
    (``save_matplotlib_png`` / ``write_plotly_html``); this class
    stays for backward compatibility with ``math_plotter.py``.
    """
